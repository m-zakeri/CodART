...
public class CorrelationAttributeEval extends ASEvaluation implements
      ...
      if (nominalIndexes.size() > 0) {
        for (Integer i : nominalIndexes) {
          ...
          for (int j = 0; j < data.attribute(i).numValues(); j++) {
            ...
            avgCorrForValue = buildEvaluatorHelper(numClasses, sumForValue == numInstances || sumForValue == 0, nomAtts[i][j], binarizedClasses, classValCounts, sumClass, sumCorr);
  ...
  public double buildEvaluatorHelper(int numClasses, boolean b, double[] y1, double[][] binarizedClasses, double[] classValCounts, double sumClass, double sumCorr) {
    double corr;
    double avgCorrForValue;
    for (int k = 0; k < numClasses; k++) {

      // corr between value j and class k
      corr = Utils.correlation(y1, binarizedClasses[k],

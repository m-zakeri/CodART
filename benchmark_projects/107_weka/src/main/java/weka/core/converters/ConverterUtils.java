...
public class ConverterUtils
    ...
    public static class DataSource
        ...
        public DataSource(String location) throws Exception {
            ...
            if (isArff(location)) {
                batchBuffer.setM_Loader(new ArffLoader());
            } else {
                if (m_File != null)
                    batchBuffer.setM_Loader(ConverterUtils.getLoaderForFile(location));
                else
                    batchBuffer.setM_Loader(ConverterUtils.getURLLoaderForFile(location));

                // do we have a converter?
                if (batchBuffer.getM_Loader() == null)
                    throw new IllegalArgumentException("No suitable converter found for '" + location + "'!");
            }

            // incremental loader?
            batchBuffer.setM_Incremental((batchBuffer.getM_Loader() instanceof IncrementalConverter));

            batchBuffer.reset(this.m_File, this.m_URL);
        }
        ...
        public DataSource(Instances inst) {
            ...
            batchBuffer.setM_BatchBuffer(inst);
            batchBuffer.setM_Loader(null);
            m_File = null;
            m_URL = null;
            batchBuffer.setM_Incremental(false);
        }
        ...
        public DataSource(Loader loader) {
            ...
            batchBuffer.setM_BatchBuffer(null);
            batchBuffer.setM_Loader(loader);
            m_File = null;
            m_URL = null;
            batchBuffer.setM_Incremental((batchBuffer.getM_Loader() instanceof IncrementalConverter));

            batchBuffer.initBatchBuffer();
        }
        ...
        public DataSource(InputStream stream) {
            ...
            batchBuffer.setM_Loader(new ArffLoader());
            try {
                batchBuffer.getM_Loader().setSource(stream);
            } catch (Exception e) {
                batchBuffer.setM_Loader(null);
            }
            m_File = null;
        ...
        public boolean isIncremental() {
            return batchBuffer.getM_Incremental();
        }
        ...
        public Loader getLoader() {
            return batchBuffer.getM_Loader();
        }
        ...
        public Instances getDataSet() throws Exception {
            return batchBuffer.getDataSet(this.m_File, this.m_URL);
        }
        ...
        public Instances getDataSet(int classIndex) throws Exception {
            ...
            result = batchBuffer.getDataSet(this.m_File, this.m_URL);
            if (result != null)
        ...
        public Instances getStructure() throws Exception {
            return batchBuffer.getStructure();
        }
        ...
        public Instances getStructure(int classIndex) throws Exception {
            ...
            result = batchBuffer.getStructure();
            if (result != null)
                result.setClassIndex(classIndex);
        ...
        public boolean hasMoreElements(Instances structure) {
            return batchBuffer.hasMoreElements(structure);
        }
        ...
        public Instance nextElement(Instances dataset) {
            return batchBuffer.nextElement(dataset);
        }
    ...
    protected static Hashtable<String, String> getFileConverters(Vector classnames, String[] intf) {
        ...
        for (i = 0; i < classnames.size(); i++) {
            ...
            for (n = 0; n < intf.length; n++) {
                ClassDiscovery.hasInterface(intf[n], classname);
            }
    ...
    public static void getFirstToken(StreamTokenizer tokenizer)
        ...
        while (true) {
            if (tokenizer.nextToken() != StreamTokenizer.TT_EOL) break;
        }
        getOtherTokenType(tokenizer);
    }
    ...
    public static void getOtherTokenType(StreamTokenizer tokenizer) {
        if ((tokenizer.ttype == '\'') ||
                (tokenizer.ttype == '"')) {

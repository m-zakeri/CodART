...
public class AprioriItemSet extends ItemSet implements Serializable,
   ...
   */
  public double leverageForRule(AprioriItemSet consequence, int premiseCount, int consequenceCount) {
  ...
  public FastVector[] generateRules(double minConfidence,
    ...
    for (int i = 0; i < m_items.length; i++) {
      generateRulesAssist(hashtables, premises, consequences, conf, lift, lev, conv, hashtable, i);
    }
    rules[0] = premises;
  ...
  public void generateRulesAssist(FastVector hashtables, FastVector premises, FastVector consequences, FastVector conf, FastVector lift, FastVector lev, FastVector conv, Hashtable hashtable, int i) {
    AprioriItemSet premise;
    AprioriItemSet consequence;
    if (m_items[i] != -1) {
      ...
      double tempLev = leverageForRule(consequence,
  ...
  public final FastVector[] generateRulesBruteForce(double minMetric,
    ...
    for (int j = 1; j < max; j++) {
      chiSquared = getSquared(minMetric, metricType, hashtables, numItemsInSet, numTransactions, significanceLevel, premises, consequences, conf, lift, lev, conv, contingencyTable, chiSquared, j);
    }
    rules[0] = premises;
    rules[1] = consequences;
    rules[2] = conf;
  ...
  public double getSquared(double minMetric, int metricType, FastVector hashtables, int numItemsInSet, int numTransactions, double significanceLevel, FastVector premises, FastVector consequences, FastVector conf, FastVector lift, FastVector lev, FastVector conv, double[][] contingencyTable, double chiSquared, int j) throws Exception {
    int numItemsInPremise;
    int help;
    Hashtable hashtableForPremise;
    ...
    if (numItemsInPremise < numItemsInSet) {
      ...
      chiSquared = getChiSquared(numTransactions, significanceLevel, premise, consequence, consequenceUnconditionedCounter, contingencyTable, chiSquared);
      ...
      if (metricType == 0) {
        ...
        if ((!(metric < minMetric))
          ...
          lift.addElement(new Double(liftForRule(premise, consequence,
              consequenceUnconditionedCounter)));
          lev.addElement(new Double(leverageForRule(consequence,
              premise.m_counter, consequenceUnconditionedCounter)));
  ...
  public double getChiSquared(int numTransactions, double significanceLevel, AprioriItemSet premise, AprioriItemSet consequence, int consequenceUnconditionedCounter, double[][] contingencyTable, double chiSquared) {
  ...
  private final FastVector[] moreComplexRules(FastVector[] rules,
    ...
    return moreComplexRulesHelper(rules, numItemsInSet, numItemsInConsequence, minConfidence, hashtables, newPremises, newConf);
  }

  private FastVector[] moreComplexRulesHelper(FastVector[] rules, int numItemsInSet, int numItemsInConsequence, double minConfidence, FastVector hashtables, FastVector newPremises, FastVector newConf) {
    FastVector newLift;
    FastVector newLev;
    FastVector newConv;
    ...
    if (numItemsInSet > numItemsInConsequence + 1) {
      ...
      while (enu.hasMoreElements()) {
        ...
        double tempLift = liftForRule(newPremise, current,
            consequenceUnconditionedCounter);
        double tempLev = leverageForRule(current,

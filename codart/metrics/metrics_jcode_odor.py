...
class JCodeOdorMetric:
    ...
    def NOCS(self, input_):
        count = 0
        classes = input_.ents('Define', 'class')
        if len(classes) > 0:
            count += len(classes)
            for cls in classes:
                count += self.NOCS(cls)
    ...
    def NOCS_Package(self, input_):
        return input_.metric(["CountDeclClass"])["CountDeclClass"]

    def NOCS_Project(self, input_):
        return input_.metric(["CountDeclClass"])["CountDeclClass"]
    ...
    def LCOM5(self, class_name):
        ...
        for enty in entities:
            for ref in enty.refs('Useby'):
                if "Constructor" not in ref.ent().kind():
                    NOACC += 1
    ...
    def give_Methods_that_the_measured_method_calls(self, funcname):
        """create a list and return it:Includes all Methods entity(also cunstructor method )
        that the measured method calls
        """

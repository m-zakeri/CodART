        ...
        if stop == len(self.tokens.tokens) - 1:
            for op in indexToOp.values():
                if op.index >= len(self.tokens.tokens) - 1:
                    buf.write(op.text)  # TODO: this check is probably not needed
    ...
    def _reduceToSingleOperationPerIndex(self, rewrites):
        ...
        for i, rop in enumerate(rewrites):
            ...
            inserts = [
                ...
                if type(op) is TokenStreamRewriter.InsertBeforeOp
            ]
            for iop in inserts:
            ...
            prevReplaces = [
                op for op in rewrites[:i] if type(op) is TokenStreamRewriter.ReplaceOp
            ]

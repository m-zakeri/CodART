...
def get_argument_parser():
    ...
    p = argparse.ArgumentParser(
        description="Helper script for ANTLR4 Swift target. "
...
def generate_spm_module(in_folder=TMP_FOLDER):
    ...
    antlr_says(
        ...
        'Put .Package(url: "{}", majorVersion: {}) in Package.swift.'.format(
            os.getcwd(), MAJOR_VERSION
        )
    )
    antlr_says(
        "(swift-tools-wersion:4.0) "
...
def generate_parser():
    ...
    if antlr is None:
        antlr_complains(
            'Run "mvn install" in antlr4 project root first or check mvn settings'
        )
        exit()

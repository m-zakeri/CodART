...
issues = repo.get_issues(
    state="closed", milestone=milestone, sort="created", direction="desc"
)
...
for x in issues:
    ...
    if x.pull_request is None and not (
        "type:improvement" in labels or "type:feature" in labels
    ):
        print(
            "* [%s](%s) (%s)"
            % (x.title, x.html_url, ", ".join([l.name for l in x.labels]))
        )

...
class common:
    ...
    @staticmethod
    def _load_vocab_from_histogram(
        path, min_count=0, start_from=0, return_counts=False
    ):
    ...
    @staticmethod
    def load_vocab_from_histogram(
        path, min_count=0, start_from=0, max_size=None, return_counts=False
    ):
        ...
        return common._load_vocab_from_histogram(
            path, min_count, start_from, return_counts
        )
    ...
    @staticmethod
    def save_word2vec_file(
        output_file, index_to_word, vocab_embedding_matrix: np.ndarray
    ):
    ...
    def calculate_max_contexts(file):
        ...
        return max(
            [
                max(l, default=0)
                for l in [
                    [len(contexts) for contexts in prog.values()]
                    for prog in contexts_per_word
                ]
            ],
            default=0,
        )
    ...
    def filter_impossible_names(special_words, top_words):
        result = list(
            filter(
                lambda word: common.legal_method_names_checker(special_words, word),
                top_words,
            )
        )
        return result
    ...
    def parse_prediction_results(
        raw_prediction_results, unhash_dict, special_words, topk: int = 5
        ...
        for single_method_prediction in raw_prediction_results:
            ...
            for i, predicted in enumerate(
                single_method_prediction.topk_predicted_words
            ):
                ...
                current_method_prediction_results.append_prediction(
                    suggestion_subtokens,
                    single_method_prediction.topk_predicted_words_scores[i].item(),
                )
            topk_attention_per_context = [
                (key, single_method_prediction.attention_per_context[key])
                for key in sorted(
                    single_method_prediction.attention_per_context,
                    key=single_method_prediction.attention_per_context.get,
                    reverse=True,
                )
            ][:topk]
            for context, attention in topk_attention_per_context:
                ...
                if hashed_path in unhash_dict:
                    ...
                    current_method_prediction_results.append_attention_path(
                        attention.item(),
                        token1=token1,
                        path=unhashed_path,
                        token2=token2,
    ...
    def squeeze_single_batch_dimension_for_np_arrays(arrays):
        assert all(
            ...
            or isinstance(array, tf.Tensor)
            for array in arrays
        )
        return tuple(
            None if array is None else np.squeeze(array, axis=0) for array in arrays
        )

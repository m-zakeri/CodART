...
_SpecialVocabWords_SeparateOovPad = Namespace(PAD="<PAD>", OOV="<OOV>")
...
class Vocab:
    def __init__(
        ...
        for index, word in enumerate(
            chain(common.get_unique_list(special_words.__dict__.values()), words)
        ):
    ...
    @classmethod
    def load_from_file(
        cls, vocab_type: VocabType, file, special_words: SpecialVocabWordsType
    ) -> "Vocab":
        special_words_as_unique_list = common.get_unique_list(
        ...
        assert (
            len(index_to_word_wo_specials)
            == len(word_to_index_wo_specials)
            == size_wo_specials
        )
        ...
        if min_word_idx_wo_specials != len(special_words_as_unique_list):
            raise ValueError(
                ...
                "Please check the parameter `config.SEPARATE_OOV_AND_PAD`.".format(
                    ...
                    file_path=file.name,
                    min_word_idx=min_word_idx_wo_specials,
                    nr_special_words=len(special_words_as_unique_list),
    ...
    @staticmethod
    def _create_word_to_index_lookup_table(
        word_to_index: Dict[str, int], default_value: int
    ):
    ...
    @staticmethod
    def _create_index_to_word_lookup_table(
        index_to_word: Dict[int, str], default_value: str
    ) -> tf.lookup.StaticHashTable:
        return tf.lookup.StaticHashTable(
            tf.lookup.KeyValueTensorInitializer(
                list(index_to_word.keys()),
    ...
    def get_word_to_index_lookup_table(self) -> tf.lookup.StaticHashTable:
        if self._word_to_index_lookup_table is None:
            self._word_to_index_lookup_table = self._create_word_to_index_lookup_table(
                self.word_to_index,
                default_value=self.word_to_index[self.special_words.OOV],
            )
...
class Code2VecVocabs:
        ...
        if self.config.is_loading:
            ...
            if not os.path.isfile(vocabularies_load_path):
                raise ValueError(
                    ...
                    "Expecting file `{vocabularies_load_path}`.".format(
                        vocabularies_load_path=vocabularies_load_path
                    )
                )
    ...
    def _load_from_path(self, vocabularies_load_path: str):
        ...
        self.config.log(
            "Loading model vocabularies from: `%s` ... " % vocabularies_load_path
        )
        with open(vocabularies_load_path, "rb") as file:
            self.token_vocab = Vocab.load_from_file(
                VocabType.Token,
                file,
                self._get_special_words_by_vocab_type(VocabType.Token),
            )
            self.target_vocab = Vocab.load_from_file(
                VocabType.Target,
                file,
                self._get_special_words_by_vocab_type(VocabType.Target),
            )
            self.path_vocab = Vocab.load_from_file(
                VocabType.Path,
                file,
                self._get_special_words_by_vocab_type(VocabType.Path),
            )
    ...
    def _create_from_word_freq_dict(self):
        ...
        self.target_vocab = Vocab.create_from_freq_dict(
            VocabType.Target,
            word_freq_dict.target_to_count,
    ...
    def _get_special_words_by_vocab_type(
        self, vocab_type: VocabType
    ) -> SpecialVocabWordsType:
    ...
    def _load_word_freq_dict(self) -> Code2VecWordFreqDicts:
        ...
        self.config.log(
            "Loading word frequencies dictionaries from: %s ... "

...
from keras_checkpoint_saver_callback import (
    ModelTrainingStatus,
    ModelTrainingStatusTrackerCallback,
...
class Code2VecModel(Code2VecModelBase):
    ...
    def _create_keras_model(self):
        ...
        context_after_dense = TimeDistributed(
            Dense(self.config.CODE_VECTOR_SIZE, use_bias=False, activation="tanh")
        )(context_embedded)
        ...
        self.keras_eval_model = keras.Model(
            inputs=inputs,
            outputs=[target_index, topk_predicted_words],
            name="code2vec-keras-model",
        )
        ...
        predict_outputs = tuple(
            KerasPredictionModelOutput(
                ...
                topk_predicted_words=topk_predicted_words,
                topk_predicted_words_scores=topk_predicted_words_scores,
            )
        )
    ...
    def _create_metrics_for_keras_eval_model(
        self,
    ) -> Dict[str, List[Union[Callable, keras.metrics.Metric]]]:
        top_k_acc_metrics = []
        ...
        words_subtokens_metrics = [
            WordsSubtokenPrecisionMetric(
                predicted_words_filters=predicted_words_filters,
        ...
        return {
            "target_index": top_k_acc_metrics,
    ...
    def _compile_keras_model(self, optimizer=None):
        ...
        self.keras_eval_model.compile(
            loss={
                "target_index": "sparse_categorical_crossentropy",
    ...
    def _create_data_reader(
        self, estimator_action: EstimatorAction, repeat_endlessly: bool = False
    ):
    ...
    def predict(self, predict_data_rows: Iterable[str]) -> List[ModelPredictionResults]:
        ...
        for input_row in input_iterator:
            ...
            input_row = _KerasModelInputTensorsFormer(
                estimator_action=EstimatorAction.Predict
            ).from_model_input_form(input_row)
    ...
    def _load_inner_model(self):
        ...
        if use_full_model:
            self.log(
                "Loading entire model from path `{}`.".format(
            ...
            self.log(
                "Loading model weights from path `{}`.".format(
    ...
    def _get_checkpoint(self):
        assert (
            self.keras_train_model is not None
            and self.keras_train_model.optimizer is not None
        )
    ...
    def _get_checkpoint_manager(self):
        if self._checkpoint_manager is None:
            self._checkpoint_manager = tf.train.CheckpointManager(
                ...
                self.config.entire_model_save_path,
    ...
    def _get_vocab_embedding_as_np_array(self, vocab_type: VocabType) -> np.ndarray:
        ...
        weight = np.array(
            self.keras_train_model.get_layer(embedding_layer_name).get_weights()[0]
        )
...
class ModelEvaluationCallback(MultiBatchCallback):
    ...
    def perform_evaluation(self):
        ...
        self.code2vec_model.log(
            "Done evaluating (took {}). Evaluation results:".format(
        ...
        self.code2vec_model.log(
            "    loss: {loss:.4f}, f1: {f1:.4f}, recall: {recall:.4f}, precision: {precision:.4f}".format(
                ...
                f1=evaluation_results.subtoken_f1,
                recall=evaluation_results.subtoken_recall,
                precision=evaluation_results.subtoken_precision,
            )
        )
        top_k_acc_formated = [
            "top{}: {:.4f}".format(i, acc)
            for i, acc in enumerate(evaluation_results.topk_acc, start=1)
        ]
...
class _KerasModelInputTensorsFormer(ModelInputTensorsFormer):
    ...
    def to_model_input_form(self, input_tensors: ReaderInputTensors):
        ...
        if self.estimator_action.is_train:
            ...
            targets = {
                "target_index": input_tensors.target_index,
    ...
    def from_model_input_form(self, input_row) -> ReaderInputTensors:
        ...
        return ReaderInputTensors(
            ...
            target_index=(
                targets if self.estimator_action.is_train else targets["target_index"]
            ),
            target_string=(
                targets["target_string"] if not self.estimator_action.is_train else None
            ),
            path_source_token_strings=(
                inputs[4] if self.estimator_action.is_predict else None
            ),
            path_strings=inputs[5] if self.estimator_action.is_predict else None,
            path_target_token_strings=(
                inputs[6] if self.estimator_action.is_predict else None
            ),
        )
...
KerasPredictionModelOutput = namedtuple(
    ...
    [
        ...
        "code_vectors",
        "attention_weights",
        "topk_predicted_words",
        "topk_predicted_words_scores",
    ],
)

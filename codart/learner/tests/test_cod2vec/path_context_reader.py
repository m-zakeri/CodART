...
class PathContextReader:
    ...
    def process_and_iterate_input_from_data_lines(
        self, input_data_lines: Iterable
    ) -> Iterable:
    ...
    def _create_dataset_pipeline(
        self, input_data_rows: Optional = None
    ) -> tf.data.Dataset:
        ...
        dataset = dataset.map(
            self._map_raw_dataset_row_to_expected_model_input_form,
    ...
    def _filter_input_rows(self, *row_parts) -> tf.bool:
        ...
        any_word_valid_mask_per_context_part = [
            tf.not_equal(
                tf.reduce_max(row_parts.path_source_token_indices, axis=0),
                self.vocabs.token_vocab.word_to_index[
                    self.vocabs.token_vocab.special_words.PAD
                ],
            ),
            tf.not_equal(
                tf.reduce_max(row_parts.path_target_token_indices, axis=0),
                self.vocabs.token_vocab.word_to_index[
                    self.vocabs.token_vocab.special_words.PAD
                ],
            ),
            tf.not_equal(
                tf.reduce_max(row_parts.path_indices, axis=0),
                self.vocabs.path_vocab.word_to_index[
                    self.vocabs.path_vocab.special_words.PAD
                ],
            ),
        ]
        any_contexts_is_valid = reduce(
            tf.logical_or, any_word_valid_mask_per_context_part
        )  # scalar
        ...
        if self.estimator_action.is_evaluate:
            ...
            word_is_valid = tf.greater(
                ...
                self.vocabs.target_vocab.word_to_index[
                    self.vocabs.target_vocab.special_words.OOV
                ],
            )  # scalar
    ...
    def _map_raw_dataset_row_to_expected_model_input_form(
        self, *row_parts
    ...
    def _map_raw_dataset_row_to_input_tensors(self, *row_parts) -> ReaderInputTensors:
        ...
        dense_split_contexts = tf.reshape(
            tf.sparse.to_dense(
                sp_input=sparse_split_contexts,
        ...
        path_source_token_strings = tf.squeeze(
            tf.slice(
                dense_split_contexts, begin=[0, 0], size=[self.config.MAX_CONTEXTS, 1]
            ),
            axis=1,
        )  # (max_contexts,)
        path_strings = tf.squeeze(
            tf.slice(
                dense_split_contexts, begin=[0, 1], size=[self.config.MAX_CONTEXTS, 1]
            ),
            axis=1,
        )  # (max_contexts,)
        path_target_token_strings = tf.squeeze(
            tf.slice(
                dense_split_contexts, begin=[0, 2], size=[self.config.MAX_CONTEXTS, 1]
            ),
            axis=1,
        )  # (max_contexts,)
        ...
        context_valid_mask = tf.cast(
            reduce(tf.logical_or, valid_word_mask_per_context_part), dtype=tf.float32
        )  # (max_contexts, )

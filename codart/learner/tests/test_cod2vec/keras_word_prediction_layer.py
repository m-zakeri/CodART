...
class WordPredictionLayer(Layer):
    ...
    def build(self, input_shape):
        if len(input_shape) != 2:
            raise ValueError(
                "Input shape for WordPredictionLayer should be of 2 dimension."
            )
    ...
    def call(self, y_pred, **kwargs):
        ...
        top_k_pred_indices = tf.cast(
            tf.nn.top_k(y_pred, k=self.top_k).indices,
            dtype=self.index_to_word_table.key_dtype,
        )
        predicted_target_words_strings = self.index_to_word_table.lookup(
            top_k_pred_indices
        )
        ...
        if self.predicted_words_filters is not None:
            masks = [
                fltr(top_k_pred_indices, predicted_target_words_strings)
                for fltr in self.predicted_words_filters
            ]
        if masks:
            ...
            legal_predicted_target_words_mask = tf.cast(
                tf.ones_like(top_k_pred_indices), dtype=tf.bool
            )
        ...
        first_legal_predicted_word_string = tf.gather_nd(
            predicted_target_words_strings, first_legal_predicted_target_word_idx
        )

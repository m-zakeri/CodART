...
class WordsSubtokenMetricBase(tf.metrics.Metric):
    ...
    def _get_true_target_word_string(self, true_target_word):
        ...
        true_target_word_index = tf.cast(
            true_target_word, dtype=self.index_to_word_table.key_dtype
        )
    ...
    def update_state(self, true_target_word, predictions, sample_weight=None):
        ...
        if sample_weight is not None:
            raise NotImplemented(
                "WordsSubtokenMetricBase with non-None `sample_weight` is not implemented."
            )
        ...
        topk_predicted_words = (
            predictions
            if self.topk_predicted_words is None
            else self.topk_predicted_words
        )
        ...
        true_target_subwords = tf.compat.v1.string_split(
            true_target_word_string, sep=self.subtokens_delimiter
        )
        prediction_subwords = tf.compat.v1.string_split(
            predicted_word, sep=self.subtokens_delimiter
        )
        true_target_subwords = tf.sparse.to_dense(
            true_target_subwords, default_value="<PAD>"
        )
        prediction_subwords = tf.sparse.to_dense(
            prediction_subwords, default_value="<PAD>"
        )
        ...
        true_target_subwords__in__prediction_subwords = tf.reduce_any(
            tf.equal(
                true_target_subwords, tf.transpose(prediction_subwords, perm=[0, 2, 1])
            ),
            axis=2,
        )
        prediction_subwords__in__true_target_subwords = tf.reduce_any(
            tf.equal(
                prediction_subwords, tf.transpose(true_target_subwords, perm=[0, 2, 1])
            ),
            axis=2,
        )

        # Count ground true label subwords that exist in the predicted word.
        batch_true_positive = tf.reduce_sum(
            tf.cast(
                tf.logical_and(
                    prediction_subwords__in__true_target_subwords,
                    prediction_subwords_mask,
                ),
                tf.float32,
            )
        )
        # Count ground true label subwords that don't exist in the predicted word.
        batch_false_positive = tf.reduce_sum(
            tf.cast(
                tf.logical_and(
                    ~prediction_subwords__in__true_target_subwords,
                    prediction_subwords_mask,
        ...
        batch_false_negative = tf.reduce_sum(
            tf.cast(
                tf.logical_and(
                    ~true_target_subwords__in__prediction_subwords,
                    true_target_subwords_mask,
                ),
                tf.float32,
            )
        )
    ...
    def _get_prediction_from_topk(self, topk_predicted_words):
        ...
        if self.predicted_words_filters is not None:
            masks = [
                fltr(topk_predicted_words) for fltr in self.predicted_words_filters
            ]
        if masks:
            ...
            legal_predicted_target_words_mask = tf.cast(
                tf.ones_like(topk_predicted_words), dtype=tf.bool
            )

        # the first legal predicted word is our prediction
        first_legal_predicted_target_word_mask = common.tf_get_first_true(
            legal_predicted_target_words_mask
        )
        first_legal_predicted_target_word_idx = tf.where(
            first_legal_predicted_target_word_mask
        )
        first_legal_predicted_word_string = tf.gather_nd(

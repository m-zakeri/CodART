...
def save_dictionaries(
    dataset_name, word_to_count, path_to_count, target_to_count, num_training_examples
):
...
def process_file(
    file_path, data_file_role, dataset_name, word_to_count, path_to_count, max_contexts
):
    sum_total = 0
    sum_sampled = 0
    total = 0
            ...
            for line in file:
                ...
                if len(contexts) > max_contexts:
                    ...
                    full_found_contexts = [
                        ...
                        if context_full_found(
                            context_parts[i], word_to_count, path_to_count
                        )
                    ]
                    partial_found_contexts = [
                        c
                        for i, c in enumerate(contexts)
                        if context_partial_found(
                            context_parts[i], word_to_count, path_to_count
                        )
                        and not context_full_found(
                            context_parts[i], word_to_count, path_to_count
                        )
                    ]
                    if len(full_found_contexts) > max_contexts:
                        ...
                        len(full_found_contexts) <= max_contexts
                        and len(full_found_contexts) + len(partial_found_contexts)
                        > max_contexts
                    ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="max_contexts",
        default=200,
        help="number of max contexts to keep",
        required=False,
    )
    parser.add_argument(
        "-wvs",
        "--word_vocab_size",
        dest="word_vocab_size",
        default=1301136,

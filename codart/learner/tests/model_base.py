...
class Code2VecModelBase(abc.ABC):
    ...
    def _init_num_of_examples(self):
        ...
        if self.config.is_training:
            ...
            self.log(
                "    Number of train examples: {}".format(
        ...
        if self.config.is_testing:
            ...
            self.log(
                "    Number of test examples: {}".format(self.config.NUM_TEST_EXAMPLES)
            )
    ...
    def _get_attention_weight_per_context(
        self,
        path_source_strings: Iterable[str],
        path_strings: Iterable[str],
        path_target_strings: Iterable[str],
        attention_weights: Iterable[float],
        ...
        for path_source, path, path_target, weight in zip(
            path_source_strings, path_strings, path_target_strings, attention_weights
        ):
            string_context_triplet = (
                common.binary_to_string(path_source),

...
def _print_round_and_scores(board):
    ...
    for p in range(n):
        print(
            f"{Style.BRIGHT}P{p}{Style.RESET_ALL}: {board.get_score(p)} points  ",
            end="",
        )
...
def _print_attribute(cell, row, use_two_rows, print_width=6):
    attributes_list = [
        attributes_sign[n_attribute]
        for n_attribute in range(14)
        if cell[n_attribute]
        for _ in range(cell[n_attribute])
    ]
    if use_two_rows:
        n = min(len(attributes_list), 6) // 2
        to_print = (
            "".join(attributes_list[:n]) if row == 0 else "".join(attributes_list[n:6])
        )
    else:
...
def _print_score_details(board):
    for player in range(board.num_players):
        ...
        scores_list = [
            (attributes_sign[n_attribute], score[n_attribute])
            for n_attribute in range(1, 14)
            if score[n_attribute]
        ]

...
class LinearNormActivation(nn.Module):
    def __init__(
        self, in_size, out_size, activation_layer, depthwise=False, channels=None
    ):
        super().__init__()
...
class SqueezeExcitation1d(nn.Module):
    def __init__(
        self, input_channels, squeeze_channels, scale_activation, setype="avg"
    ):
        super().__init__()
        if setype == "avg":
...
class InvertedResidual1d(nn.Module):
    def __init__(
        ...
        if exp_channels != in_channels:
            self.expand = LinearNormActivation(
                in_channels, exp_channels, activation_layer=activation_layer
            )
        else:
            self.expand = nn.Identity()

        # depthwise
        self.depthwise = LinearNormActivation(
            kernel,
            kernel,
        ...
        # project
        self.project = LinearNormActivation(
            exp_channels, out_channels, activation_layer=None
        )
...
class TLPNNet(nn.Module):
    def __init__(self, game, args):
        ...
        if self.version == 80:  # Very small version using MobileNetV3 building blocks
            ...
            confs += [
                InvertedResidual1d(
                    self.nb_vect, 3 * self.nb_vect, self.nb_vect, 15, False, "RE"
                )
            ]
            ...
            confs += [
                InvertedResidual1d(
                    self.nb_vect, 4 * self.nb_vect, self.nb_vect, 15, False, "RE"
                ),
                InvertedResidual1d(
                    self.nb_vect, 4 * self.nb_vect, self.nb_vect, 15, False, "RE"
                ),
            ...
            confs += [
                InvertedResidual1d(
                    self.nb_vect, 2 * self.nb_vect, self.nb_vect, 15, False, "RE"
                )
            ]
            self.trunk = nn.Sequential(*confs)
            ...
            confs += [
                InvertedResidual1d(
                    self.nb_vect, int(1.5 * self.nb_vect), self.nb_vect, 15, False, "RE"
                )
            ]
            self.trunk = nn.Sequential(*confs)
